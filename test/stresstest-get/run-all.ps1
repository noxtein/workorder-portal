...
$scenarios = @(
    @{ Name = "01 - Public Endpoints"; File = "01-public.js" },
    @{ Name = "02 - Client Endpoints"; File = "02-client.js" },
    @{ Name = "03 - Operations (SR/WO/WR)"; File = "03-operations.js" },
    @{ Name = "04 - Company & Admin"; File = "04-company.js" },
    @{ Name = "05 - Services & Config"; File = "05-services-config.js" },
...
function Parse-K6Output {
    ...
    foreach ($rawLine in $lines) {
        ...
        # Parse per-endpoint custom Trend metrics (get_*)
        if ($t -match '(get_[a-zA-Z0-9_]+)[.\s]*:.*avg=') {
            $epName = $matches[1]
            $epAvg = Get-TrendStat $t 'avg'
            $epMin = Get-TrendStat $t 'min'
            $epMed = Get-TrendStat $t 'med'
            $epMax = Get-TrendStat $t 'max'
            $epP90 = Get-TrendStat $t 'p(90)'
            $epP95 = Get-TrendStat $t 'p(95)'

            $r.EndpointStats += [PSCustomObject]@{
                Name = $epName
                Avg = $epAvg; Min = $epMin; Max = $epMax
                Med = $epMed; P90 = $epP90; P95 = $epP95
            }
        }
...
Write-Host "  GET ENDPOINTS STRESS TEST RUNNER         " -ForegroundColor Cyan
Write-Host "  100 VUs | 30s Ramp-Up | 20 Iterations   " -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════" -ForegroundColor Cyan
...
foreach ($scenario in $scenarios) {
    ...
    foreach ($ep in $parsed.EndpointStats) {
        $allEndpoints += [PSCustomObject]@{
            Scenario = $name
            Endpoint = $ep.Name
            Avg = $ep.Avg; Min = $ep.Min; Max = $ep.Max
            Med = $ep.Med; P90 = $ep.P90; P95 = $ep.P95
        }
    }
...
@{L = 'TPS'; E = { Format-Cell $_.TPS } } -AutoSize

Write-Host ""
Write-Host "═══════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "       PER-ENDPOINT STATISTICS             " -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════" -ForegroundColor Cyan

$allEndpoints | Format-Table -Property Endpoint,
@{L = 'Avg(ms)'; E = { Format-Cell $_.Avg } },
@{L = 'Min(ms)'; E = { Format-Cell $_.Min } },
@{L = 'Max(ms)'; E = { Format-Cell $_.Max } },
@{L = 'Med(ms)'; E = { Format-Cell $_.Med } },
@{L = 'P90(ms)'; E = { Format-Cell $_.P90 } },
@{L = 'P95(ms)'; E = { Format-Cell $_.P95 } } -AutoSize
...
function Build-EndpointRow {
    param([string]$label, $avg, $min, $max, $med, $p90, $p95, [string]$rowClass)

    $cls = if ($rowClass) { " class=`"$rowClass`"" } else { "" }
    $p95Cls = if ($null -ne $p95 -and $p95 -gt 3000) { " class=`"danger`"" } elseif ($null -ne $p95 -and $p95 -gt 1000) { " class=`"warning`"" } else { "" }
...
foreach ($ep in $allEndpoints) {
    $endpointRows += "`n" + (Build-EndpointRow $ep.Endpoint $ep.Avg $ep.Min $ep.Max $ep.Med $ep.P90 $ep.P95 "")
}
...
<title>GET Stress Test Report - Work Order Portal</title>
<style>
...
.header{background:linear-gradient(135deg,#1a3a2a 0%,#2d6e4e 100%);color:#fff;padding:28px 32px;border-radius:12px 12px 0 0}
.header h1{font-size:22px;font-weight:700;margin-bottom:6px}
.header .meta{color:#94c0a3;font-size:13px;line-height:1.8}
.header .meta span{color:#c3e2cf;font-weight:500}
.cards{display:grid;grid-template-columns:repeat(5,1fr);gap:0;background:#fff;border-bottom:1px solid #e2e8f0}
...
.section-title{background:#f7f9fc;padding:14px 20px;font-size:15px;font-weight:700;color:#2d3748;border-bottom:1px solid #e2e8f0;border-top:2px solid #4a6cf7}
.table-wrap{background:#fff;overflow-x:auto;box-shadow:0 2px 12px rgba(0,0,0,.08)}
.table-wrap:last-child{border-radius:0 0 12px 12px}
...
th.group-green{background:#38a169;border-color:#2f855a}
th.sub-green{background:#48bb78;border-color:#38a169}
...
<div class="header">
  <h1>&#x1F4CA; GET Endpoints Stress Test Report &mdash; Work Order Portal</h1>
  <div class="meta">
    ...
    <span>Endpoints:</span> $endpointCount &nbsp;&bull;&nbsp;
    <span>Config:</span> 100 VUs | 30s Ramp-Up | ~20 Iterations/VU
  </div>
</div>
...
<div class="cards">
  ...
  <div class="card">
    <div class="value" style="color:#2d3748">$endpointCount</div>
    <div class="lbl">Endpoints Tested</div>
  </div>
</div>
...
<thead>
<tr>
  <th class="group" rowspan="2" style="min-width:200px">Scenario</th>
...
<div class="section-title">&#x1F50D; Per-Endpoint Statistics</div>
<div class="table-wrap">
<table>
<thead>
<tr>
  <th class="group-green" rowspan="2" style="min-width:280px">Endpoint</th>
  <th class="group-green" colspan="6">Response Times (ms)</th>
</tr>
<tr>
  <th class="sub-green">Average</th>
  <th class="sub-green">Min</th>
  <th class="sub-green">Max</th>
  <th class="sub-green">Median</th>
  <th class="sub-green">90th pct</th>
  <th class="sub-green">95th pct</th>
</tr>
</thead>
<tbody>
$endpointRows
...
<div class="footer">
  Generated by k6 GET Stress Test Runner &bull; $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz')
</div>
