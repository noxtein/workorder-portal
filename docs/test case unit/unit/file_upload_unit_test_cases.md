# Skenario Pengujian Unit Test Modul File Upload
...
|---|---|---|---|---|
| TC-UNIT-FILUP-001-A | `createFiles()` | Berhasil: Mengunggah file ke sistem | Data / ID valid | Mengembalikan object record baru yang telah berhasil di-persist ke database |
| TC-UNIT-FILUP-001-B | `createFiles()` | Gagal: Validasi input salah | Data payload tidak lengkap/salah format | Melemparkan eksepsi `ValidationError` dengan detail error field |
| TC-UNIT-FILUP-001-C | `createFiles()` | Gagal: Terjadi kesalahan database/server | Mock service melempar error | Melemparkan eksepsi `InternalServerError` untuk penanganan log |
